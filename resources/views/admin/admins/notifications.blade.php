                        ...
                        <div class="email-user-list list-group ps ps--active-y">
                            <ul class="users-list-wrapper media-list">
                                    ...
                                    <li class="media mail-read">
                                        ...
                                        <div class="media-body">
                                            <div class="user-details">
                                                <a
                                                    {{ isset($notification->data['url']) ? 'href=' . $notification->data['url'] : '' }}>
                                                    <div class="mail-items">
                                                        {{-- <h5 class="list-group-item-heading text-bold-600 mb-25">{{$notification->sender['name']}}</h5> --}}
                                                        <span
                                                            class="list-group-item-text text-truncate">{{ $notification->title }}</span>
                                                    </div>
                                                </a>
                                                <div class="mail-meta-item">
                                                    <span class="float-right">
                                                        <span
                                                            class="mail-date">{{ $notification->created_at->diffForHumans() }}</span>
                                                    </span>
                                                </div>
                                            </div>
                                            <div class="mail-message mt-2" >
                                                <p style="color:#645656!important" class="list-group-item-text font-weight-normal truncate mb-0">{{ $notification->body }}</p>
                                            </div>
                                        </div>
                                    </li>
                                    <hr>
                                ...
                                <div class="no-data">
                                    ...
                                    <span class="mt-2"
                                        style="font-family: cairo">{{ __('admin.there_is_no_data_at_the_moment') }}</span>
                                </div>
                            </ul>
                    ...
                    if (usersIds.length > 0) {
                        ...
                        $.ajax({
                            ...
                            success: function(msg) {
                                ...
                                $('.checkSingle input:checked').each(function() {
                                    $(this).parents('.mail-read').next('hr').remove()
                                        .fadeOut()
