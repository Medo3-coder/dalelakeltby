...
@section('content')
    <x-admin.table datefilter="true" order="true" extrabuttons="true" addbutton="{{ route('admin.stores.create') }}"
        deletebutton="{{ route('admin.stores.deleteAll') }}" :searchArray="[
            'name' => [
                'input_type' => 'text',
...
@section('js')
    ...
    @include('admin.shared.filter_js', ['index_route' => url('admin/stores/' . request()->segment(3))])
    <script>
        $(document).ready(function() {
            $(document).on('click', '.block_user', function(e) {
                e.preventDefault();
                $.ajax({
                    url: '{{ url('admin/stores/block') }}',
                    method: 'post',
                    data: {
                        id: $(this).data('id')
                    },
