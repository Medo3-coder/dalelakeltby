            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('admin.update') . ' ' . __('admin.labcategory') }}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form method="POST"
                                action="{{ route('admin.labcategories.update', ['id' => $labcategory->id]) }}"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="controls">
                                                    <select name="type" class="select2 form-control" required
                                                        data-validation-required-message="{{ __('admin.this_field_is_required') }}">
                                                        <option value disabled>{{ __('admin.Select_the_analytics_type') }}
                                                        </option>
                                                        <option {{ $labcategory->type == 'sonar' ? 'selected' : '' }}
                                                            value="sonar">
                                                            {{ __('admin.sonar') }}</option>
                                                        <option {{ $labcategory->type == 'analytics' ? 'selected' : '' }}
                                                            value="analytics">
                                                            {{ __('admin.analytics') }}</option>

                                                        <option {{ $labcategory->type == 'scan' ? 'selected' : '' }}
                                                            value="scan">
                                                            {{ __('admin.scan') }}</option>
