    ...
    <div class="table_buttons mb-1">
        <div class="row m-0 justify-content-between">
            <div>
                @isset($addbutton)
                    @if ($addbutton != false)
                        <a href="{{ $addbutton }}" class="btn bg-gradient-primary mr-1 mb-1 waves-effect waves-light"><i
                                class="feather icon-plus"></i> {{ __('admin.add') }}</a>
                    @endif
                @endisset
                @isset($deletebutton)
                    <button type="button" data-route="{{ $deletebutton }}"
                        class="btn bg-gradient-danger mr-1 mb-1 waves-effect waves-light delete_all_button"><i
                            class="feather icon-trash"></i> {{ __('admin.delete_selected') }}</button>
                @endisset

                <button type="button" class="reload btn bg-gradient-warning mr-1 mb-1 waves-effect waves-light"><i
                        class="feather icon-refresh-cw"></i> {{ __('admin.refresh') }}</button>
            </div>
            <div>
                <div class="dt-buttons print-btns btn-group">
                    @isset($pdf)
                        <a target="_blank" href="{{ route('admin.categories.export') }}"
                            class="btn bg-gradient-success buttons-pdf buttons-html5" tabindex="0"
                            aria-controls="DataTables_Table_4">PDF</a>
                        @endif
                        @isset($excel)
                            <a target="_blank" href="{{ route('admin.categories.export') }}"
                                class="btn bg-gradient-success buttons-pdf buttons-html5" tabindex="0"
                                aria-controls="DataTables_Table_4">Excel</a>
                            @endif
                            ...
                            <div class="col-md-3 mt-1 col-12">
                                ...
                                <div class="controls border">
                                    ...
                                    @elseif ($value['input_type'] == 'select')
                                        <select name="{{ $key }}" class="form-control search-input">
                                            <option value>{{ __('admin.choose') }}</option>
                                            @foreach ($value['rows'] as $row)
                                                <option value="{{ $row['id'] }}">
                                                    {{ isset($value['row_name']) ? $row[$value['row_name']] : $row['name'] }}</option>
                                            @endforeach
                                        </select>
                                    @endif
                                </div>
