        ...
        @if ($reservations->count() != 0)
            @foreach ($reservations as $key => $reservation)
                <tr>
                    ...
                    <td class="font12">
                        <span
                            class="text-secondary">{{ $reservation->doctor_id ? $reservation->doctor->name : '--' }}</span>
                    </td>
                    ...
                    <td class="font12">
                        <button type="button"
                            @if (!in_array($reservation->status, ['approved'])) disabled 
                            @else
                            data-url="{{ route('lab.reservations.patientEnter', $reservation->id) }}" @endif
                            class="table-btn-spe main-bg up accept-patient @if (!in_array($reservation->status, ['approved'])) accept-patient follow followed followed2 @else follow unfollowed @endif  ">
                    ...
                    <td class="font12">
                        <a class="d-flex justify-content-center align-items-center"
                            href="{{ url('lab/reservation-details/' . $reservation->id) }}">@lang('site.reservation_details')</a>
                    </td>
