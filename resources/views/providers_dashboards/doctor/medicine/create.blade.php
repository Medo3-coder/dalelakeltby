    ...
    <main class="main-sec" id="main">
        <div class="container">
            <div class="card-white spe-pad">
                <h5 class="font_bold mb-4">{{ __('doctor.add_medicine') }}</h5>
                <form class="form" method="POST" action="{{ route('doctor.medicine.store') }}">
                    @csrf

                    <div class="img-cont-spe">
                        <img src="{{$settings['default_image']}}" alt="" id="change-profile">
                        <label class="edit-spe" for="img-up"><i class="fa-regular fa-pen-to-square"></i>
                            <input name="image" onchange="loadFiles(event)" type="file" id="img-up" hidden="">
                        </label>
                    </div>
                    ...
                    <div class="mb-3 main-inp-cont col-12 col-lg-8">
                        ...
                        <div class="form__label mt-0">
                            <input class="default_input" name="name" type="text" required=""
                                placeholder="{{ __('admin.please_enter') }} {{ __('admin.name') }}" />
                    ...
                    <div class="mb-3 main-inp-cont col-12 col-lg-8">
                        ...
                        <div class="form__label mt-0">
                            <input class="default_input" name="type" type="text" required=""
                                placeholder="{{ __('doctor.please_select_medicine_type') }}" />
                            <label class="float__label"
                                for="">{{ __('doctor.please_select_medicine_type') }}</label>
                        </div>
                    </div>
                    <button class="submit submit-button wid-60 up mt-5">{{ __('admin.add') }}</button>
                </form>
            </div>
    </main>
@endsection
