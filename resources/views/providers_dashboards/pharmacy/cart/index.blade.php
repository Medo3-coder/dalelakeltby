                                ...
                                <div class="card_foot card-f-spe">
                                    ...
                                    <div class="flex-device justify-content-center">
                                        ...
                                        <a href="#" class="link-device up makeOrderGetData"
                                            data-url="{{ route('pharmacy.cart.makeOrderGetData', $store_id) }}"
        ...
        <div class="modal-dialog modal-dialog-spe2">
            <div class="modal-content">
                <div class="modal-body no-border-bottom modal2-spe text-center">
                    ...
                    <div class="font_bold don-t">@lang('localize.order_made_successfuly') </div>
...
@push('js')
    ...
    <script>
        ...
        $(document).on('click', '[data-change-cart]', function() {
            var url = "{{ route('pharmacy.cart.change') }}"
            $.ajax({
                url: url,
                method: 'post',
                data: {
        ...
        $(document).on('click', '.deleteProduct', function() {
            ...
            var url = "{{ route('pharmacy.cart.deleteProduct') }}"
            $.ajax({
                url: url,
                method: 'post',
                data: {
        ...
        $(document).on('click', '.check-coupon', function() {
            var url = "{{ route('pharmacy.cart.saveCoupon') }}";
            let old_content;
            const btn = $(this)
        ...
        $(document).on('click', '.delete-coupon', function() {
            var url = "{{ route('pharmacy.cart.deleteCoupon') }}";
            let old_content;
            const btn = $(this)
            $.ajax({
                url: url,
