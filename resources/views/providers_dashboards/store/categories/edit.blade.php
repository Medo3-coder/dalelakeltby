    ...
    <main class="main-sec" id="main">
        <div class="container">
            <div class="side-heading">
                <h6>{{ __('store.edit_category', ['name'=>$additive->name]) }}</h6>
                <div class="links-top-to">
                    <a href="{{ route('store.categories') }}">{{ __('store.Products categories') }}</a>  /
                    <span class="color-main">{{ __('store.edit_category', ['name'=>$additive->name]) }}</span>
                </div>
            </div>
            <form action="{{ route('store.categories.update', $additive->id) }}" id="form" method="POST" enctype="multipart/form-data" class="form">
                @csrf
                @method('PUT')
                <div class="card-ins">
