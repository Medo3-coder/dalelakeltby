        ...
        <div class="container">
            ...
            <div class="card-white font-17">
                ...
                <div class="img-cont-spe">
                    <img src="{{ authUser('store')->image }}" alt="">

                </div>
                <div class="col-lg-8 col-12 flex-info font_bold">
                    <div>{{ __('store.name') }} : <span class="font-700">{{ authUser('store')->name }}</span></div>
                    <div>{{ __('store.Mobile number') }} : <span class="font-700">{{ authUser('store')->fullPhone }}</span>
                    </div>
                </div>
                <div class="col-lg-8 col-12">
                    <div class="profile-in font_bold mt-3 mb-3">{{ __('store.email') }} : <span
                            class="font-700">{{ authUser('store')->email }}</span></div>
                </div>
                <div class="col-lg-8 col-12">
                    <div class="profile-in font_bold mb-3">{{ __('store.ID Number') }} : <span
                            class="font-700">{{ authUser('store')->identity_number }}</span></div>
                </div>

                <div class="col-lg-8 col-12">
                    <div class="profile-in font_bold mb-3">{{ __('store.Delivery price') }} : <span
                            class="font-700">{{ authUser('store')->delivery_price . ' ' . __('store.Dinar') }}</span></div>
                </div>
            ...
            <div class="card-white font-17">
                <div class="top-card-flex">
                    ...
                    <button class="add-per up" type="button" data-toggle="modal"
                        data-target="#exampleModal">{{ __('store.permit_add') }}</button>
                </div>
                ...
                @forelse($permits as $permit)
                    ...
                    <a href="#" onclick="deletePermit(this)" data-val="{{ $permit->id }}"><i
                            class="fa-solid fa-trash deletePermit"></i></a>
            ...
            <div class="card-white font-17">
                <div class="top-card-flex">
                    <a href="#" data-toggle="modal" data-target="#exampleModal2"><i
                            class="fa-solid fa-pen-to-square"></i></a>
                    <h4 class="font_bold no-border-bottom font-17">{{ __('store.Repository data') }}</h4>
                </div>
                <div class="profile-in">{{ __('store.store address') }} : <span>{{ $firstBranch->address }}</span></div>
                <div class="profile-in">{{ __('store.Store address on the map') }} :
                    <span>{{ $firstBranch->address_map }}</span></div>
                <div class="profile-in">{{ __('store.Repository record number') }} :
                    <span>{{ $firstBranch->comerical_record }}</span></div>
                    ...
                    <div class="left-flex-btn font-14">
                        ...
                        @forelse($firstBranch->dates()->orderBy('day', 'ASC')->get() as $date)
                            <div class="ll-flex-card">
                                ...
                                <div class="day-spe">{{ __('store.from') }}
                                    {{ Carbon\Carbon::parse($date->from)->translatedFormat('g:i A') }}
                                    {{ __('store.to') }} {{ Carbon\Carbon::parse($date->to)->translatedFormat('g:i A') }}
                                </div>
                            </div>
                        @empty
                        @endforelse
                ...
                <h4 class="font_bold mt-3 mb-3">{{ __('store.stock photos') }}</h4>
            ...
            <button class="delete-spe  up delete-account"
                onclick="deleteAccount(this)">{{ __('store.delete account') }}</button>
        </div>
    </main>

    @include('providers_dashboards.store.profile.includes.personal-data')
    @include('providers_dashboards.store.profile.includes.permit')
    @include('providers_dashboards.store.profile.includes.branches')
...
@push('js')
    ...
    @include('providers_dashboards.store.includes.js.profile')
@endpush
