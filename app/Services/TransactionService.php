...
//  use NotifyTrait;
    ...
    public function walletPaySuccess($reservation) {
        /**
         * with success pay
        ...
        $type = 'newWalletReservationPaySuccess';

        # cut from user wallet
        $this->cutFromWallet($user, $reservation->final_total, $type, $reservation);

        # record getting money in admin wallet untill finish order
        $this->addToWallet($this->superAdmin, $reservation->final_total, $type, $reservation);
    ...
    public function updateBalance($transaction): void {
        if ($transaction->dept) {
            $transaction->transactionable->update(['wallet_balance' => $transaction->transactionable->wallet_balance - $transaction->dept]);
        } elseif($transaction->credit) {
            $transaction->transactionable->update(['wallet_balance' => $transaction->transactionable->wallet_balance + $transaction->credit]);
        }
    }
}